    ...
    <div className="product-shell">
      ...
      <section className="main-area">
        ...
        {view==="tagging" ? <div className="workspace-board" ref={boardRef}>
          ...
          <section className="fm-panel canvas-panel">
            <header><div><span>ILLUSTRATOR</span><h1>{matchName}</h1></div><div className="illustrator-actions"><button onClick={()=>{setIllustrations(current=>current.slice(0,-1));setSelectedIllustration(null)}} disabled={!illustrations.length}>UNDO</button><button onClick={deleteSelectedIllustration} disabled={selectedIllustration===null}>DELETE</button><button className="fm-primary" onClick={()=>void exportAnnotatedVideo()} disabled={exporting||!videoUrl||!sessionClips.length}>{exporting?"EXPORTING…":"EXPORT VIDEO »"}</button></div></header>
            <div ref={illustratorRef} className={`illustrator-canvas tool-${tool.toLowerCase().replaceAll(" ","-")}`} onPointerDown={beginIllustration} onPointerMove={moveIllustration} onPointerUp={finishIllustration} onPointerCancel={()=>{setIllustrationDraft(null);setIllustrationMove(null)}}>
              {videoUrl?<video ref={videoRef} className="illustrator-video" src={videoUrl} muted playsInline onTimeUpdate={e=>setClock(Math.floor(e.currentTarget.currentTime))}/>:<div className="video-grid"><MiniPitch/></div>}
              ...
              {selectedDrawing&&<button className="object-delete" style={{left:`${selectedDrawing.start.x}%`,top:`${selectedDrawing.start.y}%`}} onPointerDown={e=>e.stopPropagation()} onClick={deleteSelectedIllustration} aria-label="Delete selected annotation">×</button>}
              {!illustrations.length&&!illustrationDraft&&!polygonDraft.length&&<div className="canvas-empty"><b>Choose a drawing tool</b><small>Annotations stay linked to this match clip.</small></div>}
            </div>
            <footer><button onClick={togglePlay}>{playing?"Ⅱ PAUSE":"▶ PLAY"}</button><div><span style={{width:`${videoRef.current?.duration?Math.min(100,clock/videoRef.current.duration*100):0}%`}}/></div><b>{formattedClock}</b></footer>
          </section>
          <aside className="illustrator-side">
            <section className="fm-panel compact-tools"><header><div><span>DRAW</span><h1>Annotations</h1></div><button className="clear-drawings" onClick={()=>{setIllustrations([]);setSelectedIllustration(null)}} disabled={!illustrations.length}>CLEAR ALL</button></header><div className="tool-strip">{illustratorTools.map(t=><button key={t} title={t} aria-label={t} className={tool===t?"selected":""} onClick={()=>{setTool(t);if(t!=="Area")setPolygonDraft([])}}><i>{t==="Arrow"?"↗":t==="Curved arrow"?"⌁":t==="Link line"?"╱":t==="Distance"?"↔":t==="Area"?"△":t==="Text"?"T":t==="Select"?"↖":"○"}</i><span>{t}</span></button>)}</div>{tool==="Area"&&polygonDraft.length>=3&&<button className="finish-area" onClick={finishPolygon}>FINISH {polygonDraft.length}-POINT AREA</button>}<div className="style-row"><label>LINE <input type="color" value={selectedDrawing?.color||illustrationLine} onChange={e=>{setIllustrationLine(e.target.value);updateSelectedIllustration({color:e.target.value})}}/></label><label>FILL <input type="color" value={selectedDrawing?.fill||illustrationFill} onChange={e=>{setIllustrationFill(e.target.value);updateSelectedIllustration({fill:e.target.value})}}/></label><label>SIZE <input aria-label="Annotation size" type="range" min="45" max="180" value={selectedDrawing?.size||illustrationSize} onChange={e=>{const value=Number(e.target.value);setIllustrationSize(value);updateSelectedIllustration({size:value})}}/></label><label>OPACITY <input type="range" min="10" max="100" value={selectedDrawing?.opacity||illustrationOpacity} onChange={e=>{const value=Number(e.target.value);setIllustrationOpacity(value);updateSelectedIllustration({opacity:value})}}/></label>{selectedDrawing?.tool==="Text"&&<label>TEXT <input className="illustrator-text-input" value={selectedDrawing.text||""} onChange={e=>updateSelectedIllustration({text:e.target.value})}/></label>}</div><div className="illustrator-help"><b>{tool}</b><span>{tool==="Area"?"Click 3 vertices and finish, or click a fourth vertex.":tool==="Select"?"Select, drag or resize an object. Delete removes only that object.":tool==="Player ring"||tool==="Spotlight"||tool==="Text"?"Click directly on the player.":"Press, drag and release."}</span></div></section>
            <section className="fm-panel illustrator-events"><header><div><span>LIVE EVENT LOG</span><h1>{events.length} match events</h1></div></header><div className="illustrator-event-list">{events.map(event=><button key={event.id} className={selectedEventId===event.id?"selected":""} onClick={()=>seekToEvent(event)}><time>{event.minute}</time><i style={{background:event.color}}>{event.action[0]}</i><span><b>{event.action}</b><small>{event.player} · {event.outcome}</small></span><strong>›</strong></button>)}</div><div className="clip-maker"><div><label>PRE <input type="number" min="0" max="30" value={clipPreRoll} onChange={e=>setClipPreRoll(Number(e.target.value))}/></label><label>POST <input type="number" min="0" max="30" value={clipPostRoll} onChange={e=>setClipPostRoll(Number(e.target.value))}/></label></div><button onClick={createClip} disabled={selectedEventId===null}>＋ CREATE CLIP</button></div></section>
          </aside>
          <section className="fm-panel playlist-panel"><header><div><span>CLIPS & PLAYLISTS</span><h1>Organize the analysis</h1></div><div className="new-playlist"><input aria-label="New playlist name" value={newPlaylistName} placeholder="New playlist" onChange={e=>setNewPlaylistName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")createPlaylist()}}/><button onClick={createPlaylist}>＋</button></div></header><div className="playlist-columns">{playlists.map(list=><section key={list.id}><header><b>{list.name}</b><small>{sessionClips.filter(clip=>clip.playlistIds.includes(list.id)).length} clips</small></header>{sessionClips.filter(clip=>clip.playlistIds.includes(list.id)).map(clip=><button key={clip.id} onClick={()=>{const video=videoRef.current;if(video)video.currentTime=clip.start;setClock(Math.floor(clip.start))}}><span>{clip.title}</span><small>{Math.max(0,clip.end-clip.start).toFixed(1)}s</small></button>)}</section>)}</div>{sessionClips.length>0&&<div className="clip-library"><b>CLIP LIBRARY</b>{sessionClips.map(clip=><div key={clip.id}><button onClick={()=>void exportAnnotatedVideo(clip)}>▶ {clip.title}</button><span>{playlists.map(list=><label key={list.id}><input type="checkbox" checked={clip.playlistIds.includes(list.id)} onChange={()=>toggleClipPlaylist(clip.id,list.id)}/>{list.name}</label>)}</span><button aria-label={`Delete ${clip.title}`} onClick={()=>setClips(current=>current.filter(item=>item.id!==clip.id))}>×</button></div>)}</div>}</section>
        </div>}
      </section>
    </div>
